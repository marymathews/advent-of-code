...
class AocDayFour {
	...
	bool ValidatePasswordData(Dictionary < string, string > password) {
		...
		foreach(string key in password.Keys) {
			if (!IsFieldValid(key, password[key])) validFlag = false;
		}
		return validFlag;
	}
	...
	void CountValidPuzzles(List < string > input) {
		...
		Dictionary < string,
		string > fields = new Dictionary < string,
		string > ();
		foreach(string line in input) {
			if (String.IsNullOrEmpty(line)) {
				if (ValidatePasswordFields(fields)) {
					countExcludingDataValidation++;
					if (ValidatePasswordData(fields)) countIncludingDataValidation++;
				}
				fields = new Dictionary < string,
				string > ();
			}
			else {
	...
	bool ValidateHgt(string value) {
		if (value.EndsWith("cm")) {
			...
			if (integerValue >= 150 && integerValue <= 193) return true;
		}
		else if (value.EndsWith("in")) {
			int integerValue = Convert.ToInt32(value.Substring(0, value.Length - 2));
			if (integerValue >= 59 && integerValue <= 76) return true;
		}
		return false;
	}
